    ...
    do // run program until user quits
    {
        ...
        else if (user_choice == 1)
        {
            ...
            else // if username not found in list, user can create password
            {
                do
                {
                    printf("\nEnter a password (8-15 characters): "); // ask user for password
                    scanf("%s", pass);
        ...
        {
            bool in_user = false;  // determines if username in program
            user_t *index = users; // pointer to iterate through each user
            char friend_user[30];
            friend_t *friend;

            printf("Enter a friend's username to view their posts: ");
            scanf("%c", friend_user);

            while (index) // iterate through list for each user
            {
                if (strcmp(index->username, friend_user) == 0) // determine if users username is found in system by comparing to each username in system
                {
                    in_user = true; // if username is found, exit loop and set variable to true to indicate it
                    //friend->username = friend_user;
                    break;
                }
                index = index->next; // iterate to next user
            }

            display_posts_by_n(friend->posts, NUM_POSTS); // display posts in intervals of 3 as instructed
            break;                                // return to main menu
        }
        case 5: // exit program
