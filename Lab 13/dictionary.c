...
{
    for (int i = 0; i < TABLESIZE; i++) // iterate through each index of hashtable
    {
        printf("\n%d: ", i);      // print the index number
        entry_t *index = dict[i]; // get index of linked list
        if (index != NULL)        // if the index has a value and is not null
        {
            do
            {
                printf("%s: %s ", index->key, index->value); // print the name and email
                if (index->next != NULL)                     // if the index has more then one entry, print a comma for a break
                    printf(", ");
                index = index->next; // go to next index (or entry)
            } while (index != NULL); // keep printing until no more index's left to print
        }
        else
            printf("NULL"); // if index is NULL or empty, print NULL to indicate this
    }
}
...
{
    int index = hash(key);                       // find index of given key in the hashtable
    entry_t *replace = search(dict[index], key); // use search to search list at the index in hashtable to find target entry to be replaced

    if (replace == NULL)
        return false; // return false for unsucessfull replacement
    else
    {
        free(replace->value);           // free the index
        replace->value = strdup(value); // fill index with its associated string pointing to by value (replace old index with new)
        assert(replace->value != NULL); // terminate via assert if memory could not be allocated while updating dictionary
        return true;                    // return true for succesfull replacement
    }
}
...
{
    for (int i = 0; i < TABLESIZE; i++) // iterate through each index of hastable
    {
        entry_t *index = dict[i]; // get index of linked list
        while (index != NULL)     // if the index has a value and is not null
        {
            free(index->key);    // empty key of index
            free(index->value);  // empty value of index
            free(index);         // empty index itself
            index = index->next; // go to next index
        }
        dict[i] = NULL; // ensure dictionary is empty
    }
}